...
namespace ManagerServer
{
    ...
    {
        private static async Task<IEnumerable<Model.Object>> Upgrade87(Orm.SQLiteConnection objects, IProgress<Tuple<int, int>> progress)
        {
            await Task.CompletedTask;
            var list = new List<Model.Object>();
            foreach (var e in objects.OfType<Model.Obsolete.Obsolete26.SalesInvoiceTemplate26>().ToArray())
            {
                if (!string.IsNullOrWhiteSpace(e.Obsolete_Notes)) e.Obsolete_Notes = e.Obsolete_Notes.Replace("\r\n", "\r\n\r\n");
