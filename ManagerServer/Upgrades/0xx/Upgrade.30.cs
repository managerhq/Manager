...
namespace ManagerServer
{
    ...
    {
        private static async Task<IEnumerable<Model.Object>> Upgrade30(Orm.SQLiteConnection objects, IProgress<Tuple<int, int>> progress)
        {
            await Task.CompletedTask;
            var list = new List<Model.Object>();
            var salesQuotes = objects.OfType<Model.Obsolete.Obsolete02.SalesQuote02>().ToArray();
            foreach (var e in salesQuotes)
            {
                var o = new Model.SalesQuote();
                o.Key = e.Key;
