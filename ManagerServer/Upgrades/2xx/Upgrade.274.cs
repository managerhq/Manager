...
namespace ManagerServer
{
    ...
    {
        private static async Task<IEnumerable<Model.Object>> Upgrade274(Orm.SQLiteConnection objects, IProgress<Tuple<int, int>> progress)
        {
            await Task.CompletedTask;
            var list = new List<ManagerServer.Model.Object>();
            foreach (var e in objects.OfType<ManagerServer.Model.DebitNote>())
            {
                if (e.Lines == null && e.Obsolete_Lines != null)
                {
                    var lines = new List<DebitNote.Line>();
