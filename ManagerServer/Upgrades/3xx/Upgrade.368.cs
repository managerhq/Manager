...
namespace ManagerServer
{
    ...
    {
        private static async Task<IEnumerable<Model.Object>> Upgrade368(Orm.SQLiteConnection objects, IProgress<Tuple<int, int>> progress)
        {
            ...
            foreach (var e in objects.OfType<BillableTime>().Where(x => x.Customer.HasValue && foreignCurrencyCustomers.ContainsKey(x.Customer.Value)).ToArray())
            {
            ...
            foreach (var e in objects.OfType<WithholdingTaxReceipt>().Where(x => x.Customer.HasValue && foreignCurrencyCustomers.ContainsKey(x.Customer.Value)).ToArray())
            {
                var currency = foreignCurrencyCustomers[e.Customer.Value];
                var exchangeRate = objects.OfType<ExchangeRate>().Where(x => x.Currency == currency && x.Date <= e.Date && (x.Obsolete_BaseRate > 0m || x.Obsolete_CounterRate > 0m)).OrderByDescending(x => x.Date).FirstOrDefault();
                if (exchangeRate != null)
