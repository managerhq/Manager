...
namespace ManagerServer
{
    ...
    {
        private static async Task<IEnumerable<Model.Object>> Upgrade363(Orm.SQLiteConnection objects, IProgress<Tuple<int, int>> progress)
        {
            await Task.CompletedTask;
            var list = new List<ManagerServer.Model.Object>();
            foreach (var e in objects.OfType<JournalEntry>().Where(x => x.Currency.HasValue).ToArray())
            {
